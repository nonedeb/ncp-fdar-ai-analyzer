...
def create_app():
    ...
    def student_submit():
        if request.method == "POST":
            ...
            extracted_text = extract_pdf_text(path) if ext == "pdf" else "Image uploaded. OCR path can be added next."
            analysis = analyze_with_openai(document_type, extracted_text or "No extractable text found.")
            s = Submission(
                student_id=session["user_id"], document_type=document_type, title=title, original_filename=filename, file_path=path,
                extracted_text=extracted_text, ai_total_score=float(analysis.get("total_score",0)), ai_breakdown_json=json.dumps(analysis.get("breakdown",{})),
                ai_feedback=analysis.get("feedback_summary"), ai_strengths=json.dumps(analysis.get("strengths",[])),
                ai_improvements=json.dumps(analysis.get("improvements",[])), ai_suggested_revision=analysis.get("suggested_revision"),
                ai_curriculum_tags=json.dumps(analysis.get("curriculum_tags",[])), final_score=float(analysis.get("total_score",0)), status="analyzed"
            )
